...
public class Publisher extends JFrame implements Runnable {

    //	static Notify message = new Notify();
    //RTP variables:
    //----------------
...
//            timer.stop();
    ...
    public void play(String fileName, String user) {

//    	int imagenb = 0;
        //map，保存订阅用户的视频传输记录

        int imagenb = map.get(user);
        ...
        while (imagenb < VIDEO_LENGTH) {
            imagenb++;
            try {
                int image_length = video.getnextframe(publisher.buf);
                RTPpacket rtp_packet = new RTPpacket(MJPEG_TYPE, imagenb, imagenb*FRAME_PERIOD,
                        publisher.buf, image_length);
...
//            StringBuffer mes = new StringBuffer("RTSP/1.0 200 OK"+CRLF);
//            mes.append("CSeq: "+RTSPSeqNb+CRLF);
//            mes.append("Session: "+RTSPid+CRLF);
//            trans.send.
...
//            mes.append("CSeq: "+RTSPSeqNb+CRLF);
//            mes.append(des);
//            message.messageNotify(mes.toString());
//            System.out.println("RTSP Server - Sent response to Client.");
//        } catch(Exception ex) {
//            System.out.println("Exception caught: "+ex);
//            System.exit(0);
